...
while True:
    ...
    pro_id = input("请输入省编号,或输入q(quit)退出：").strip()   #省ID
    if pro_id.isdigit():
        pro_id = int(pro_id)
        if pro_id > 0 and pro_id <= len(province_list):
            ...
            while True:
                ...
                city_id = input("请输入市编号,或输入b(back)返回上级菜单，或输入q(quit)退出：").strip()
                if city_id.isdigit():
                    city_id = int(city_id)
                    if city_id > 0 and city_id <= len(city_list):
                        ...
                        while True:
                            ...
                            back_or_quit = input("输入b(back)返回上级菜单，或输入q(quit)退出：").strip()
                            if back_or_quit == 'b':
